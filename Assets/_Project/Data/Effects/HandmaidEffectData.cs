...
public class HandmaidEffect : CardEffect
{
    public override void Resolve(GameState game, PlayerState source, PlayerState target, int? guessValue)
    {
        ...
        Debug.Log($"Player {source.id + 1} is protected until their next turn.");
        TurnLogger.Instance.Log($"Player {source.id + 1} is protected until their next turn.", game.turnNumber);
    }
}